...
void WorkerThread::set_pointers(QUdpSocket* inp_pSock, Dispatcher* inp_pDispatcher,
                                ...
                                std::mutex* inp_pSock_mtx, bool* inp_pNextAsleep, std::mutex* inp_pNextAsleep_mtx)
{
    ...
    pWakeNext = inp_pWakeNext;
    pWake_mtxNext = inp_pWake_mtxNext;

    pSock_mtx = inp_pSock_mtx;

    pNextAsleep = inp_pNextAsleep;
    pNextAsleep_mtx = inp_pNextAsleep_mtx;
}
...
void WorkerThread::run()
{
    ...
    {
        asleep_mtx.lock();
        asleep = false;//Report to prev worker
        asleep_mtx.unlock();
        ...
        if(pRcvSocket->hasPendingDatagrams())
        {
            ...
            if(datagramSize == INPUT_SIZE)
            {
                ...
                if(filterIntput(received[0]))
                {
                    ...
                    if(timer.nsecsElapsed() < INTERVAL_BTWN_OUTP)//Since prev thread last send
...
void WorkerThread::kickNextWorker()
{
    pNextAsleep_mtx->lock();
    while(!*pNextAsleep)//to prevent falling asleep while next one is STILL awake
    {
        pNextAsleep_mtx->unlock();
    ...
    while(*pNextAsleep)
    {
        ...
        std::lock_guard<std::mutex> lk(*pWake_mtxNext);
        pWakeNext->notify_all();
        std::this_thread::sleep_for(std::chrono::nanoseconds(1000));
